...
int median_of_medians(std::vector<int> &nums, size_t batch_size) {
    ...
    for (size_t i = 0; i < nums.size(); i += batch_size) {
        std::vector<int> batch(nums.begin() + i, nums.begin() + std::min(i + batch_size, nums.size()));
...
int find_nth_element(std::vector<int> &nums, size_t n, bool use_better_median, size_t batch_size) {
    ...
    if (n < partition.first.size()) {
        ...
        return find_nth_element(partition.second,
                                n - partition.first.size() -
                                    mediant_estimate_count, use_better_median, batch_size); // -1 because we already counted the median_estimate
    }
}

double find_median_good(const std::vector<int> &nums, bool use_sorting, bool use_better_median, size_t batch_size) {
    if (use_sorting) {
        std::sort(nums.begin(), nums.end());
    if (nums.size() % 2 == 1) {
    ...
    if (nums.size() % 2 == 1) {
        return static_cast<double>(find_nth_element(nums, (nums.size() - 1) / 2, use_better_median, batch_size));
    }
    double lo = static_cast<double>(find_nth_element(nums, (nums.size() - 2) / 2, use_better_median, batch_size));
    double hi = static_cast<double>(find_nth_element(nums, nums.size() / 2, use_better_median, batch_size));
    return (lo + hi) / 2.0;
}
...
int main() {
    ...
    std::vector<std::string> catergories = {"sorting", "quickselect", "quickselect + better median (5)","quickselect + better median (7)","quickselect + better median (9)", "quickselect + better median (101)"};
    ...
    for (size_t i = 0; i < catergories.size(); i++) {
        for (size_t SIZE = 10; SIZE <= 100000000; SIZE *= 2) {
            nums.clear();
